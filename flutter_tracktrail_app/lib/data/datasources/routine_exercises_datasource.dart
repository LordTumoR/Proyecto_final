...
class RoutineExerciseRemoteDataSourceImpl
  ...
  Future<ExerciseModel> createExercise(
    ...
    if (exercise.idExercise == 0 || exercise.idExercise == null) {
      response = await client.post(
        Uri.parse('http://10.250.76.46:8080/exercises/$routineId/$userId'),
        headers: {
      ...
      response = await client.put(
        Uri.parse('http://10.250.76.46:8080/exercises/${exercise.idExercise}'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  Future<List<RoutineExerciseModel>> getAllRoutineExercises() async {
    ...
    final response = await client.get(
      Uri.parse('http://10.250.76.46:8080/routine-exercises'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
  ...
  Future<void> updateRoutineExerciseCompletion(
    ...
    const String token = 'admin';
    final url = 'http://10.250.76.46:8080/routine-exercises/$routineExerciseId';
