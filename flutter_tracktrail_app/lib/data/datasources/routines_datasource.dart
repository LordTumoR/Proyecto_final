...
class RoutineRemoteDataSourceImpl implements RoutineRemoteDataSource {
  ...
  Future<List<RoutineModel>> getRoutines() async {
    ...
    final response = await client.get(
      Uri.parse('http://192.168.1.142:8080/routines'),
      headers: {
  ...
  Future<RoutineModel> createRoutine(
    ...
    if (routineId != null && routineId != 0) {
      final response = await client.put(
        Uri.parse('http://192.168.1.142:8080/routines/$routineId'),
        headers: {
          'Authorization': 'Bearer $token',
  ...
  Future<void> deleteRoutine(int idRoutine) async {
    ...
    final response = await client.delete(
      Uri.parse('http://192.168.1.142:8080/routines/$idRoutine'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
  ...
  Future<double> getCompletion(int routineId) async {
    ...
    final response = await client.get(
      Uri.parse(
          'http://192.168.1.142:8080/routine-exercises/$routineId/completion'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
